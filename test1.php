...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "UPDATE cms SET cms_status=? WHERE cms_id=?";
    $stmt = $conn->prepare($sql);

    if ($stmt) {
        $stmt->bind_param("sssii",  $status, $id);
        if ($stmt->execute()) {
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="form-check form-switch">
                                            <input class="form-check-input cms-status-switch" type="checkbox" data-cms-id="<?php echo $row['cms_id']; ?>" <?php echo ($row['cms_status'] == 1) ? "checked" : ""; ?>>
